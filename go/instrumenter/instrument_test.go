...
func TestProbesSimpleDeclarations(t *testing.T) {
	...
	if !strings.Contains(result.Generated, `price := 40; __ziglive_probe(`) {
		t.Fatalf("price probe missing:\n%s", result.Generated)
	}
	if !strings.Contains(result.Generated, `"price", price)`) {
		t.Fatal("probe arguments missing")
	}
	if !strings.Contains(result.Generated, `var tax int = 3; __ziglive_probe(`) {
	...
	if strings.Contains(result.Generated, "i := 0; __ziglive_probe") {
...
func TestLogMarkersTrackLoops(t *testing.T) {
	...
	if !strings.Contains(result.Generated, `__ziglive_log_loop(1, 7, 11, 3, 10, 2, "i", i)`) {
...
func TestManualModeInsertsOnlySelected(t *testing.T) {
	...
	if strings.Contains(none.Generated, "__ziglive_probe") {
...
func TestInstrumentedSourcePassesThrough(t *testing.T) {
	source := "package main\n\nfunc main() { x := 1; __ziglive_probe(\"a\", 1, 1, \"x\", x) }\n"
