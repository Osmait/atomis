...
Your code has no network by default. **Settings → Allow network** opens outbound connections for the program itself — `fetch` and HTTP clients work — while the sandbox keeps confining the filesystem and still refuses to let anything listen. Remember to raise the execution timeout too: a real request rarely fits in the default 2 s.

Work lives in **workspaces**. A *scratch session* (the default) is temporary — its directory is created under the OS temp dir and removed when you leave. A **persistent workspace** keeps everything: open the switcher from the status bar (`⌂ scratch`) or the tree's ⋯ menu, name one, and its files (and its toolchain caches) live under `$XDG_DATA_HOME/atomis/workspaces/<id>` until you delete it. Reloading reattaches to the last one you opened; each workspace is a separate project, so you can keep several experiments side by side.
...
- Vim Mode is enabled by default and can be toggled in the settings modal (⌘,). Use `i` to insert, `Esc` for Normal mode and `:w` to run the current source. Native Ctrl/Cmd+A/C/X/V shortcuts and the editor's right-click Copy/Paste menu remain available. On top of monaco-vim's full keymap (motions, operators, text objects, visual block, marks, registers, macros, `/` search, `:s`), Atomis adds: **f/t assistance (clever-f)** — nothing shows until you type the motion's character; then every match of that character in the document stays lit while `;`/`,` repeat the jump (the motion itself stays line-scoped, as in vim) — plus LSP/editor keys `gd` (definition), `gr` (references), `K` (hover docs), `gcc`/visual `gc` (comment), `za`/`zc`/`zo`/`zR`/`zM` (folding), visual `=` (LSP format), `ZZ` (run), ex commands `:e <file>` (open/create), `:bd` (close tab), `:only` (close the others), and **surround** under the `gs` namespace — `gsw<char>` wraps the word, `gss<char>` the line, visual `gs<char>` the selection, `gsd<char>` deletes the enclosing pair and `gsc<char>` swaps it (the new character is a single keypress into the status bar); typing the opening character pads with inner spaces, vim-surround style.
