...
Eligible local `const`/`var` declarations receive a same-line call immediately after their semicolon. Direct `std.debug.print` and `std.log` statements are followed by a private source marker in the generated copy. Node strips that marker and attaches the original line and column to the matching terminal output. Insertions are sorted by descending original byte offset. The generated file keeps every original byte in order and exactly the same newline count; visible `src/main.zig` is never changed by probes and is what Monaco/ZLS use.
