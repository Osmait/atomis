...
Eligible local `const`/`var` declarations receive a same-line call immediately after their semicolon. Direct `std.debug.print` and `std.log` statements are followed by a private source marker in the generated copy. Node strips that marker and attaches the original line, column, and per-call execution number to the matching terminal output. For logs enclosed by `for` or `while`, the AST adapter also records the innermost loop line and a detected payload, continuation, or condition variable; its runtime value is rendered with the same bounded preview rules. Insertions are sorted by descending original byte offset. The generated file keeps every original byte in order and exactly the same newline count; visible `src/main.zig` is never changed by probes and is what Monaco/ZLS use.
